...
describe('meal-plans page template', () => {
  it('renders hero, collapsible calendar, and inventory check modal structure', () => {
    ...
    expect(template).toContain('calendar-nav__button-icon')
  ...
  it('uses animated calendar viewport styles and modal cards instead of a floating fab list', () => {
    ...
    expect(styles).toMatch(/\.calendar-grid\s*\{[\s\S]*gap:\s*10rpx;/)
    expect(styles).toMatch(/\.calendar-nav__button-icon\s*\{[\s\S]*width:\s*18rpx;[\s\S]*height:\s*18rpx;[\s\S]*border-right:/)
    ...
    expect(styles).toMatch(/\.calendar-cell\s*\{[\s\S]*height:\s*88rpx;[\s\S]*box-sizing:\s*border-box;[\s\S]*border-radius:\s*22rpx;[\s\S]*background:\s*rgba\(31,\s*42,\s*54,\s*0\.055\);/)
    expect(styles).toMatch(/\.calendar-cell--has-plans\s*\{[\s\S]*background:\s*var\(--surface-muted,/)
    expect(styles).toMatch(/\.calendar-cell--outside-month\s*\{[\s\S]*background:\s*rgba\(31,\s*42,\s*54,\s*0\.028\);/)
    expect(styles).toMatch(/\.calendar-cell--outside-month\.calendar-cell--has-plans\s*\{[\s\S]*background:\s*var\(--surface-muted,/)
  ...
  it('uses theme variables for the meal plans hero and page surface', () => {
    ...
    expect(styles).not.toContain('#35754a')
    expect(styles).not.toContain('86, 163, 108')
    expect(styles).not.toContain('53, 117, 74')
    expect(styles).toContain('var(--brand-strong, #1b5e20)')
    expect(styles).toContain('rgba(46, 125, 50')
    expect(styles).toContain('rgba(27, 94, 32')

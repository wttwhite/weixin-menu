...
describe('pantry edit template', () => {
  ...
  it('keeps notes textarea and delete action compact instead of full width', () => {
    ...
    expect(styles).toMatch(/\.quantity-stepper\s*\{[\s\S]*gap:\s*0;/)
    expect(styles).toMatch(/\.quantity-stepper\s*\{[\s\S]*overflow:\s*hidden;/)
    expect(styles).toMatch(/\.quantity-stepper\s*\{[\s\S]*background:\s*#fafbfe;/)
    expect(styles).toMatch(/\.stepper-button\s*\{[\s\S]*border-radius:\s*0;/)
    expect(styles).toMatch(/\.stepper-button\s*\{[\s\S]*background:\s*#fafbfe;/)
    expect(styles).toMatch(/\.stepper-button\s*\{[\s\S]*color:\s*#667084;/)
    expect(styles).toMatch(/\.stepper-value\s*\{[\s\S]*padding:\s*0 16rpx;/)
    expect(styles).toMatch(/\.stepper-value\s*\{[\s\S]*border-left:\s*1px solid #edf0f4;/)
    expect(styles).toMatch(/\.stepper-value\s*\{[\s\S]*border-right:\s*1px solid #edf0f4;/)
  })
})

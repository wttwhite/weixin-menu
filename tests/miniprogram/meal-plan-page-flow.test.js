...
describe('meal-plans page flow', () => {
  ...
  it('builds a month calendar and filters plans by selected day', async () => {
    ...
    expect(page.data.selectedPlans[0].statusText).toBe('待执行')
  })
  ...
  it('opens recipe detail from a plan recipe tag and opens edit page from the edit tool', async () => {
    const callFunction = vi.fn().mockResolvedValue({
      result: {
        code: 0,
        data: {
          items: [
            {
              ...
              recipes: [
                { recipeId: 'recipe-1', recipeNameSnapshot: '番茄蛋花汤', recipe: { _id: 'recipe-1', name: '番茄蛋花汤' } },
                { recipeId: 'recipe-2', recipeNameSnapshot: '凉拌黄瓜', recipe: { _id: 'recipe-2', name: '凉拌黄瓜' } }
              ]
            }
          ]
        }
    ...
    expect(navigateTo).toHaveBeenNthCalledWith(1, {
      url: '/pages/recipe-detail/index?recipeId=recipe-2&from=plans'
    })
    expect(navigateTo).toHaveBeenNthCalledWith(2, {
      url: '/pages/meal-plan-edit/index?mealPlanId=meal-1'
    })
  })

  it('opens the status modal, updates meal plan status, and supports deleting the selected plan', async () => {
    const callFunction = vi
      .fn()
      .mockResolvedValueOnce({
        result: {
            ...
            items: [
              {
                ...
                status: 'planned',
                notes: '',
                recipes: [{ recipeId: 'recipe-1', recipeNameSnapshot: '番茄蛋花汤', recipe: { _id: 'recipe-1', name: '番茄蛋花汤' } }]
              }
            ]
          }
        }
    ...
    page.handleToggleMealPlanStatus({
      currentTarget: {
        dataset: {
          mealPlanId: 'meal-1'
        }
    ...
    expect(callFunction).toHaveBeenNthCalledWith(2, {
      ...
      data: {
        ...
        plan: {
          ...
          notes: '',
          recipes: [
            { recipeId: 'recipe-1', servingsOverride: '', notes: '' }
          ]
        }
      },
      config: undefined
    ...
    expect(page.data.showMealPlanStatusModal).toBe(true)
    expect(page.data.selectedPlans[0]).toEqual(
      expect.objectContaining({
        statusText: '已完成',
        statusIconText: '✓'
      })
    )
    expect(page.data.mealPlanStatusChipItems).toEqual([
      expect.objectContaining({
        value: 'planned',
        active: false,
        itemClass: 'meal-plan-status-chip'
      }),
      expect.objectContaining({
        value: 'done',
        active: true,
        itemClass: 'meal-plan-status-chip meal-plan-status-chip--active'
      }),
      expect.objectContaining({
        value: 'cancelled',
        active: false,
    ...
    await page.removeSelectedMealPlan()
    ...
    expect(page.data.showMealPlanStatusModal).toBe(false)
    expect(page.data.selectedPlans).toEqual([])
    expect(showToast).toHaveBeenCalledWith({
      title: '已删除计划',
      icon: 'success'
  ...
  it('highlights the tapped status immediately before the cloud update resolves', async () => {
    ...
    const submitPromise = page.selectMealPlanStatus({
      currentTarget: {
        dataset: {
          status: 'done'
        }
    ...
    expect(page.data.mealPlanStatusModalStatus).toBe('done')
    expect(page.data.mealPlanStatusChipItems[1]).toEqual(
      expect.objectContaining({
        value: 'done',
        active: true,
        itemClass: 'meal-plan-status-chip meal-plan-status-chip--active'
...
describe('meal-plan edit page flow', () => {
  it('filters recipe options by keyword and selects a recipe through the search overlay', async () => {
    const callFunction = vi
      .fn()
      .mockResolvedValueOnce({
        result: {
          ...
          data: {
            items: [
              { _id: 'recipe-1', name: '鸡汤' },
              { _id: 'recipe-2', name: '凉拌黄瓜' },
              { _id: 'recipe-3', name: '番茄蛋花汤' }
            ]
          }
        }
      })
    ...
    expect(page.data.showRecipeSelector).toBe(true)
    expect(page.data.activeRecipeRowIndex).toBe(0)

    page.handleRecipeSearchInput({
      detail: {
        value: '黄瓜'
      }
    })

    expect(page.data.filteredRecipeOptions).toEqual([
      expect.objectContaining({
        label: '凉拌黄瓜',
        value: 'recipe-2'
      })
    ...
    expect(page.data.form.recipes[0].recipeId).toBe('recipe-2')
    expect(page.data.selectedRecipeIndexes[0]).toBe(2)
    expect(page.data.showRecipeSelector).toBe(false)
  })
  ...
  it('preserves loaded status when saving an existing meal plan', async () => {
    const callFunction = vi
      .fn()
      .mockResolvedValueOnce({
        result: {

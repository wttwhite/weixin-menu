...
describe('custom tab bar', () => {
  it('uses a full-width docked layout instead of a floating inset shell', () => {
    ...
    expect(styles).toMatch(/\.tabbar-surface\s*\{[\s\S]*border-radius:\s*0;/)
    expect(styles).toMatch(/\.tabbar-surface\s*\{[\s\S]*padding:\s*14rpx 18rpx calc\(env\(safe-area-inset-bottom\) \+ 10rpx\);/)
    expect(styles.includes('tabbar-item__pill')).toBe(false)
    expect(styles.includes('tabbar-item__glow')).toBe(false)
    expect(styles.includes('translateY(-4rpx)')).toBe(false)
    expect(template.includes('tabbar-item__pill')).toBe(false)
    expect(template.includes('tabbar-item__glow')).toBe(false)

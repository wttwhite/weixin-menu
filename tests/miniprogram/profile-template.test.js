...
describe('profile page template', () => {
  it('renders profile hero, space card, grouped actions, and management modals', () => {
    ...
    expect(template).toContain('manager-modal')
  ...
  it('uses grouped management cards and theme chips instead of a dense icon wall', () => {
    ...
    expect(styles).toMatch(/\.manager-modal,\s*\.theme-modal\s*\{[\s\S]*align-items:\s*center;/)
    expect(styles).toMatch(/\.manager-modal__panel,\s*\.theme-modal__panel\s*\{[\s\S]*border-radius:\s*34rpx;/)

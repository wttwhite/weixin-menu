...
describe('shopping page template', () => {
  ...
  it('renders status tabs, shopping list cards, and inline modals', () => {
    ...
    expect(template).toContain('bindtap="archiveShoppingList"')
    expect(template).toContain('wx:if="{{item.canArchive}}"')
    expect(template).toContain('shopping-list-card__action--archive')
    expect(template).not.toMatch(/>\s*归档\s*<\/view>/)
    expect(template.includes('handleListStatusChange')).toBe(false)
    expect(template.includes('listForm.status')).toBe(false)
  ...
  it('uses a scrollable compact shopping modal form with aligned manual item controls', () => {
    ...
    expect(styles).toMatch(/\.shopping-modal\s*\{[\s\S]*height:\s*100%;[\s\S]*max-height:\s*82vh;/)
    expect(styles).toMatch(/\.shopping-modal__body\s*\{[\s\S]*flex:\s*1;[\s\S]*min-height:\s*0;[\s\S]*height:\s*auto;[\s\S]*overflow-y:\s*auto;/)
    expect(styles).toMatch(/\.shopping-modal__footer\s*\{[\s\S]*flex:\s*none;/)
